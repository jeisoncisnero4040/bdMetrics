...
class MetricsService:
    ...
    def processRecord(self, db_name: str):
        ...
        for query in frequent:
            ...
            queries_total.labels(database=db_name, table=table).set(query['execution_count'])
            cpu_total.labels(database=db_name, query=q_text, table=table).set(query.get('cpu_time_total', 0))
            duration_total.labels(database=db_name, query=q_text, table=table).set(query.get('duration_total', 0))
            logical_reads_total.labels(database=db_name, query=q_text, table=table).set(query.get('logical_reads_total', 0))
            logical_writes_total.labels(database=db_name, query=q_text, table=table).set(query.get('logical_writes_total', 0))
            physical_reads_total.labels(database=db_name, query=q_text, table=table).set(query.get('physical_reads_total', 0))
            exec_per_second.labels(database=db_name, query=q_text, table=table).set(query.get('exec_per_second', 0))
